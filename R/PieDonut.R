...
PieDonut=function(data,mapping,
  ...
  # Inserisci qui il codice per applicare i colori personalizzati
  if (!is.null(customColors)) {
    # Assicurati che 'pies' sia fattorizzato e che i livelli corrispondano a quelli in 'customColors'
    df[[pies]] <- factor(df[[pies]], levels = names(customColors))
    # Assegna i colori da 'customColors' basandoti sui livelli di 'pies'
    mainCol <- customColors[levels(df[[pies]])]
  } else {
    # Generazione predefinita dei colori se 'customColors' non è fornito
  ...
  if(!is.null(donuts)){
    ...
    if(explodeDonut) {
      p3<-p+geom_arc_bar(aes_string(x0 = "x", y0 = "y", r0 = as.character(r1),
                                    ...
                                    fill = pies,explode="focus"),alpha=donutAlpha,color=color,
                         data = df3)
    } else{
      p3<-p+geom_arc_bar(aes_string(x0 = "x", y0 = "y", r0 = as.character(r1),
                                    r = as.character(r2), start="start1",end="end1",
                                    fill = pies),alpha=donutAlpha,color=color,
                         data = df3)
    }
